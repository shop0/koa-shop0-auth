...
const DEFAULT_MYSHOP0_DOMAIN = "myshop0.com";
export const DEFAULT_ACCESS_MODE: AccessMode = "online";

export const TOP_LEVEL_OAUTH_COOKIE_NAME = "shop0TopLevelOAuth";
export const TEST_COOKIE_NAME = "shop0TestCookie";
export const GRANTED_STORAGE_ACCESS_COOKIE_NAME =
  "shop0.granted_storage_access";
...
export default function createshop0Auth(options: OAuthStartOptions) {
  const config = {
    prefix: "",
    myshop0Domain: DEFAULT_MYSHOP0_DOMAIN,
  ...
  return async function shop0Auth(ctx: Context, next: NextFunction) {
    ...
    if (
      ...
      const redirectUrl = await shop0.Auth.beginAuth(
        ctx.req,
        ctx.res,
        shop,
        oAuthCallbackPath,
    ...
    if (ctx.path === oAuthCallbackPath) {
      try {
        await shop0.Auth.validateAuthCallback(ctx.req, ctx.res, ctx.query);

        ctx.state.shop0 = await shop0.Utils.loadCurrentSession(
          ctx.req,
          ctx.res,
          config.accessMode === "online"
        );
        ...
        switch (true) {
          case e instanceof shop0.Errors.InvalidOAuthError:
            ctx.throw(400, e.message);
            break;
          case e instanceof shop0.Errors.SessionNotFound:
