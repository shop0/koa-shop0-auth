...
import { fetch } from "@shop0/jest-dom-mocks";
...
const TEST_SHOP = "testshop.myshop0.io";
const TEST_USER = "1";
...
describe("verifyRequest", () => {
  ...
  describe("when there is an accessToken and shop in session", () => {
    let jwtToken: string;
    const jwtSessionId = shop0.Auth.getJwtSessionId(TEST_SHOP, TEST_USER);

    beforeEach(async () => {
      const jwtPayload = {
        iss: `https://${TEST_SHOP}/admin`,
        dest: `https://${TEST_SHOP}`,
        aud: shop0.Context.API_KEY,
        sub: TEST_USER,
      ...
      jwtToken = jwt.sign(jwtPayload, shop0.Context.API_SECRET_KEY, {
        algorithm: "HS256",
      });

      const session = new shop0.Session.Session(jwtSessionId);
      session.shop = TEST_SHOP;
      session.expires = new Date(jwtPayload.exp * 1000);
      session.accessToken = "test_token";
      session.scope = "test_scope";
      await shop0.Utils.storeSession(session);
    });
    ...
    it("calls next for offline sessions", async () => {
      const session = new shop0.Session.Session(
        shop0.Auth.getOfflineSessionId(TEST_SHOP)
      );
      session.shop = TEST_SHOP;
    ...
    it("redirects to the given authRoute if the shop in session does not match the one in the query param", async () => {
      ...
      const next = jest.fn();
      const ctx = createMockContext({
        url: appUrl("some_other_shop.myshop0.io"),
        redirect: jest.fn(),
        headers: { authorization: `Bearer ${jwtToken}` },
      });
      ...
      expect(ctx.redirect).toHaveBeenCalledWith(
        `${authRoute}?shop=some_other_shop.myshop0.io`
      );
    });
    ...
    it("redirects to authRoute if the session scopes are different than the current setting", async () => {
      ...
      const session = await shop0.Context.SESSION_STORAGE.loadSession(
        jwtSessionId
      );
  ...
  describe("when there is no session", () => {
    ...
    it("does not fail to clear the current session", async () => {
      ...
      const jwtToken = jwt.sign(jwtPayload, shop0.Context.API_SECRET_KEY, {
        algorithm: "HS256",
      });
...
async function expireJwtSession(sessionId: string) {
  const session = await shop0.Context.SESSION_STORAGE.loadSession(sessionId);
